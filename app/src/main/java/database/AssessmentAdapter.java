...
import com.gregsnyder.courseappwguc196.AssessmentDetailsActivity;
import com.gregsnyder.courseappwguc196.AssessmentAddEditActivity;
...
public class AssessmentAdapter extends RecyclerView.Adapter<AssessmentAdapter.ViewHolder> {
    ...
    public void setAssessmentSelectedListener(AssessmentAdapter.AssessmentSelectedListener assessmentSelectedListener) {
        this.assessmentSelectedListener = assessmentSelectedListener;
    }
    ...
    public void onBindViewHolder(@NonNull AssessmentAdapter.ViewHolder holder, int position) {
        ...
        holder.tvDate.setText(DateTextFormatting.cardDateFormat.format(assessment.getDate()));
        ...
        switch(rContext) {
            case MAIN:
                ...
                holder.assFab.setOnClickListener(v -> {
                    Intent intent = new Intent(mContext, AssessmentAddEditActivity.class);
                    intent.putExtra(ASSESSMENT_ID_KEY, assessment.getId());
                    mContext.startActivity(intent);
                });
                break;
